...
export function ProcessTimeline({ onOpenBooking }: ProcessTimelineProps) {
  return (
    <section className="w-full py-16 md:py-24 bg-[#fbf9f6]" id="process">
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {PROCESS_STEPS.map((s, idx) => (
            <div
              key={idx}
              className="flex flex-col gap-4 bg-white p-6 sm:p-7 rounded-2xl border border-[#e4e2df] shadow-sm justify-between"
            >
              <div>
                <div className="flex items-center gap-4 mb-4">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center font-serif text-xl font-semibold shadow-sm ${
                      s.isPrimary
                        ? "bg-[#d4a574] text-[#2c1700]"
                        : "bg-[#efeeeb] text-[#1b1c1a]"
                    }`}
                  >
                    {s.step}
                  </div>
                  <span className="h-px flex-1 bg-[#eae8e5]" />
                </div>
              ...
              <div className="p-3 rounded-xl bg-[#f5f3f0] text-[10px] font-mono text-[#827569] uppercase font-semibold border border-[#e4e2df]">
                {s.badge}
              </div>
            </div>
          ))}
