...
export function TestimonialsSection() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && reviewsGridRef.current) {
    ...
    if (reviewsGridRef.current) {
      observer.observe(reviewsGridRef.current);
    }
  ...
  return (
    <section className="w-full py-16 md:py-24 bg-[#f1f5f9]" id="testimonials">
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 lg:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            ...
            <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl text-[#0f172a] font-light tracking-tight">
              HEAR WHAT OUR <span className="italic font-serif text-[#3b71ad]">CUSTOMERS SAY</span>
            </h2>
          ...
          <div className="bg-white px-5 py-3 rounded-2xl border border-[#e2e8f0] shadow-xs flex items-center gap-3">
            <div className="flex flex-col">
              ...
              <span className="font-mono text-[11px] text-[#0f172a] font-bold mt-1">
        ...
        <div ref={reviewsGridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {TESTIMONIALS.map((review, idx) => (
            <div
              key={idx}
              className="bg-white p-7 rounded-2xl border border-[#e2e8f0] shadow-xs flex flex-col justify-between hover:shadow-md transition-shadow"
            >
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="font-mono text-[10px] text-[#64748b]">{review.time}</span>
                </div>

                <p className="font-serif text-sm font-medium text-[#0f172a] mb-2 leading-snug">
                  "{review.highlight}"
                </p>
                ...
                {review.text.length > 160 && (
                  <button
                    ...
                    className="text-[11px] font-mono text-[#3b71ad] hover:underline font-semibold cursor-pointer mb-4"
                  >
              ...
              <div className="pt-3 border-t border-[#f1f5f9] flex items-center justify-between">
                <div>
                  ...
                  <div className="text-[10px] font-mono text-[#64748b]">{review.location}</div>
                </div>
                <span className="text-[10px] font-mono text-[#3b71ad] bg-[#ebf3fc] px-2.5 py-0.5 rounded-full font-medium border border-[#80a8d8]/20">
                  Verified Client
                </span>
              </div>
            </div>
        ...
        <div className="text-center mt-10">
          <a
            ...
            className="inline-flex items-center gap-2 text-xs font-mono text-[#3b71ad] hover:text-[#0f172a] font-bold uppercase tracking-wider transition-colors"
          >
